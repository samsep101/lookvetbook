    ...
    class DoctorController extends BaseController
    {
        ...
        public function get()
        {
            ...
            else
            {
                ...
                $all_reviews             = $doctor_review_manager->getConfirmedListByDoctorId($doctor->getId());
            ...
            if(!empty($doctor))
            {
                $this->setRecentlyViewedDoctor($doctor->getId());
                $doctor_to_slider = $this->getDoctorsToSlider($doctor);
            }
        ...
        private function getDoctorsToSlider($doctor)
        {
            ...
            else
            {
                ...
                $doctor_for_clinic = array();
                $clinics_names     = array();

                foreach($clinics AS $clinic)
                {
                    $doctor_search_params            = new DoctorSearchParams();
                    $doctor_search_params->clinic_id = $clinic->getId();
                    $clinics_names[]                 = $clinic->name;

                    $doctor_for_clinic = array_merge($doctor_for_clinic, $doctor_manager->getResultsForRequestCriteria($doctor_search_params));

                    unset($doctor_search_params);
                }

                $specialties          = $doctor->specialties;
                $doctor_for_specialty = array();
                ...
                foreach($specialties AS $specialty)
                {
                    ...
                    $doctor_for_specialty = array_merge($doctor_for_specialty, $doctor_manager->getResultsForRequestCriteria($doctor_search_params));
                ...
                foreach($doctor_for_clinic AS $dfcValue)
                {
                    if(in_array($dfcValue, $doctor_for_specialty))
                    {
                        $result[] = $dfcValue;
                    }
                }

                $doctors = $doctor_manager->getDoctorsTheListOfIdentifiers($result);
                $data    = array(
            ...
            if(empty($doctors))
            {
                ...
                $doctor_for_region    = array();
                $doctor_for_specialty = array();

                foreach($clinics AS $clinic)
                {
                    $doctor_search_params            = new DoctorSearchParams();
                    $doctor_search_params->region_id = $clinic->region_id;

                    $doctor_for_region = array_merge($doctor_for_region, $doctor_manager->getResultsForRequestCriteria($doctor_search_params));
                ...
                $other_doctors_ids = array();
                foreach($doctor_for_specialty AS $dfsValue)
                {
                    if(in_array($dfsValue, $doctor_for_region))
                    {
                        $other_doctors_ids[] = $dfsValue;
                    }

                    if(count($other_doctors_ids) == 50) break;
                }
                ...
                $doctors = $doctor_manager->getDoctorsTheListOfIdentifiers($other_doctors_ids);
                $title   = $this->getDoctorBlockTitle($data);
            }
        ...
        public function index($specialty_alias = NULL)
        {
...
//            $search_params = clone $doctor_search_params;
//            $search_params->page = $doctor_search_params->page;
//            $search_params->by_page = $doctor_search_params->by_page;
			...
			if($city || $street_id || $region_id || $district_id)
			{
            ...
            $this->view->specialtyIDForDoctorCard = $specialty->id;
            ...
            {
                $topNumberH1      = SeoTextViewHelper::getTopNumberH1('', $city);
                $defaultSpecialty = 1;
            }
            else
            {
