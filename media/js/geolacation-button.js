    ...
    ymaps.util.augment(GeolocationButton, ymaps.control.Button, {
        ...
        onAddToMap: function () {
            ...
            ymaps.option.presetStorage.add('geolocation#icon', {
                iconImageHref: '/media/images/man.png',
                iconImageSize: [27, 26],
        ...
        _onBtnClick: function (e) {
            ...
            this.toggleIconImage('/media/images/loader.gif');
        ...
        _onGeolocationSuccess: function (position) {
            ...
            this.toggleIconImage('/media/images/wifi.png');
        },
        /**
        ...
        handleGeolocationResult: function (position) {
            //var this_place = ymaps.geolocation.city+', '+ymaps.geolocation.region;
            citymap.points = '';
            var this_place = ymaps.geolocation.city;
            Ajax.Post('/ajax/getCityIdByCityName', {city_name:this_place}, function (data) {
                if (data.result.city_id) {
                    controller.form_controller.latitude = position.coords.latitude;
                    controller.form_controller.longitude = position.coords.longitude;
                    controller.form_controller.map_controller.latitude = position.coords.latitude;
                    controller.form_controller.map_controller.longitude = position.coords.longitude;
                    city_controller.setCityInfo(data.result.city_id, position.coords.latitude,position.coords.longitude);
                    $('.map-city').html(ymaps.geolocation.city);

                    //controller.form_controller.map_controller.setViewRange();
                }
                else {
                    if (controller.form_controller.container == '#doctor-search-form')
                        $('#our-doctors').html('<div class="error-plate">У нас пока нет врачей в городе '+ymaps.geolocation.city+'. Мы сообщим, как только они появятся!</div>');
                    else
                        $('#our-doctors').html('<div class="error-plate">У нас пока нет клиник в городе '+ymaps.geolocation.city+'. Мы сообщим, как только они появятся!</div>');
                    $('.map-city').html(ymaps.geolocation.city);
                }
            });
