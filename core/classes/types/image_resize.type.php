<?php
    ...
    class ImageResizeType extends Type
    {
        ...
        public function getFormValue($val = '')
        {
            ...
            $link = '<a href="/media/upload/' . $this->fieldInfo['base_dir'] . $this->getValue() . '">' . ($this->getValue()) . '</a>';

            $result = <<<EOD
			<table width="100%" cellpadding="3" cellspacing="3" border="0" style="border:1px solid #e4e4e4">
			<tr>
			        <td colspan="1" class="td_main" valign="top">
			                <input type="file" name="{$this->fieldName}" class=""> <span class="grey">Имя файла должно иметь только латинские символы и цифры.</span>
...
EOD;
            if (!empty($nFileSize)) {
				...
				<tr>
				        <td width="30%" class="td_main"><span class="grey">Размер:</span> {$nFileSize}.
				        </td>
				        <td class="td_main">
				                <span class="grey">Путь:</span> {$link}<br>
								<span class="grey">Имя файла:</span> {$this->value}
				        </td>
