...
(function ($, undefined) {
    ...
    var defaults = {
        ...
        isRTL:false,
        firstDay:0,
        monthNames:['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Ибль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'],
                ...
                for (j = 0; j < colCnt; j++) {
                    s +=
                        "<td class='fc- " + contentClass + " fc-day" + (i * colCnt + j) + "'>" + // need fc- for setDayID
                            ...
                            "<div>" +
                            //Доработка для добаления событий
                            (showNumbers ?
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            for (i = 0; d < maxd; i++) {
                hours = d.getMinutes();
                s +=
                    "<tr class='fc-slot" + i + ' ' + (!hours ? '' : 'fc-minor') + "'>" +
                        "<th class='fc-agenda-axis " + headerClass + "'>" +
                        ((!slotNormal || !hours) ? formatDate(d, opt('axisFormat')) : '&nbsp;') +
                        "</th>" +
        ...
        function timePosition(day, time) { // both date objects. day holds 00:00 of current day
            ...
            var slotMinutes = opt('slotMinutes'),
                ...
                slotI = Math.floor(hours / slotMinutes),
                slotTop = slotTopCache[slotI];
            if (slotTop === undefined) {
                slotTop = slotTopCache[slotI] = slotTable.find('tr:eq(' + slotI + ') td div')[0].offsetTop; //.position().top; // need this optimization???
            }
            return Math.max(0, Math.round(
                slotTop - 1 + slotHeight * ((hours % slotMinutes) / slotMinutes)
            ));
        }
    ...
    function DayEventRenderer() {
        ...
        function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                if ($(event).attr('delete') == '0') {
                    html +=
                        " style='background:#99CCFF; color:#333333; font-weight:bold'";
                }
                html +=
...
//			if(event.delete != '0'){
                if ($(event).attr('delete') != '0') {
                    html +=
                        "<img src='/media/images/deleteIcon.png' title='Удалить' border='0' align='absmiddle' onclick='removeEvent(" + event.id + ")' />&nbsp;";
                }
                html +=
